...
curl https://$FUNCTION_URL/api/health | jq .

echo "Testing /api/parameters..."
curl https://$FUNCTION_URL/api/parameters | jq .
```
...
**IMPORTANT for demo scripts:** If using `demo/contract/*.sh` scripts, set:
```bash
export CONTRACT_URL=https://<your-function-app>.azurewebsites.net/api
```

Note the `/api/` suffix! Demo scripts call `$CONTRACT_URL/parameters` which becomes `/api/parameters`.
...
This endpoint is required by demo scripts and depa-training workflows to establish the "root of trust" for contract validation.
...
**Purpose:**
- Used by `demo/contract/1-contract-setup.sh` to create trust store
- Used by `depa-training` resource provisioning scripts
- Enables `validate-contract` command to test validation workflow

**Note:** Returns static mock parameters. The certificate matches:
- `mock_trust_store/mock-blob-storage-service.json`
- Receipts generated by `/api/submit`
- Python CLI mock certificate

Receipts will parse correctly but fail cryptographic verification (intentional for training).
