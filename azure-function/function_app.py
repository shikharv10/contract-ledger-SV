...
This Azure Function provides HTTP endpoints for submitting contracts to blob storage
with atomic sequence number assignment using blob leases, and for retrieving mock
service parameters for trust store setup.
...
Endpoints:
    ...
    GET /api/parameters - Get mock service parameters for trust store
...
# Mock service parameters
# These MUST match the certificate in mock_trust_store/mock-blob-storage-service.json
# and the certificate used in pyscitt/pyscitt/cli/submit_signed_contract.py
...
def get_parameters(req: func.HttpRequest) -> func.HttpResponse:
    ...
    - Demo scripts (demo/contract/1-contract-setup.sh)
    ...
    The parameters establish the "root of trust" for verifying contracts and receipts.
    For blob storage mode, these are MOCK parameters for training/demo purposes only.
    ...
    Response format matches CCF /parameters endpoint:
    {
      ...
      "treeAlgorithm": "CCF",
      "signatureAlgorithm": "ES256",
      "serviceCertificate": "MIIDbTCCA..."  # Base64-encoded DER certificate
    }

    The certificate in the response matches:
    - mock_trust_store/mock-blob-storage-service.json
    - Certificates in receipts generated by /api/submit
    - MOCK_CERTIFICATE_DER in pyscitt/pyscitt/cli/submit_signed_contract.py

    Note: These parameters do NOT provide cryptographic guarantees.
          Receipts will parse correctly but fail signature verification.
          This is intentional for training/demo use cases.
    """
    logging.info('Serving mock service parameters')

    return func.HttpResponse(
        json.dumps(MOCK_SERVICE_PARAMETERS),
        status_code=200,
        mimetype="application/json"
    )
